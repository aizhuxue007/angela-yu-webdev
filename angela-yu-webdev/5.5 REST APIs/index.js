...
const yourBearerToken = "1e7d5e99-47ff-4d01-8f6f-ea694b99e53b";
const config = {
...
app.post("/put-secret", async (req, res) => {
  ...
  try {
    const resp = await axios.put(API_URL + '/secrets/' + searchId, req.body, config)
    res.render('index.ejs', { content: JSON.stringify(resp.data) })
  } catch (error) {
    res.render('index.ejs', { content: JSON.stringify(error.response.data) })
  }
...
app.post("/delete-secret", async (req, res) => {
  ...
  try {
    ...
    res.render('index.ejs', { content: JSON.stringify(resp.status) })
  } catch (error) {
    res.render('index.ejs', { content: JSON.stringify(error.response.data) })
  }
});
